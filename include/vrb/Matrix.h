...
namespace vrb {
    ...
    public:
        ...
        static Matrix
        Rotation(const Vector &aDirection, const Vector &aUp = vrb::Vector(0.0f, 1.0f, 0.0f)) {
            Matrix result;
        ...
        bool IsIdentity() const {
            return VRB_IS_ZERO(m.m00 - 1.0f) && VRB_IS_ZERO(m.m01) && VRB_IS_ZERO(m.m02) &&
                   VRB_IS_ZERO(m.m03)
                   && VRB_IS_ZERO(m.m10) && VRB_IS_ZERO(m.m11 - 1.0f) && VRB_IS_ZERO(m.m12) &&
                   VRB_IS_ZERO(m.m13)
                   && VRB_IS_ZERO(m.m20) && VRB_IS_ZERO(m.m21) && VRB_IS_ZERO(m.m22 - 1.0f) &&
                   VRB_IS_ZERO(m.m23)
                   && VRB_IS_ZERO(m.m30) && VRB_IS_ZERO(m.m31) && VRB_IS_ZERO(m.m32) &&
                   VRB_IS_ZERO(m.m33 - 1.0f);
        }

        Vector MultiplyPosition(const Vector &aPosition) const {
            Vector result;
            result.x() =
                    m.m00 * aPosition.x() + m.m10 * aPosition.y() + m.m20 * aPosition.z() + m.m30;
            result.y() =
                    m.m01 * aPosition.x() + m.m11 * aPosition.y() + m.m21 * aPosition.z() + m.m31;
            result.z() =
                    m.m02 * aPosition.x() + m.m12 * aPosition.y() + m.m22 * aPosition.z() + m.m32;
            const float w =
                    m.m03 * aPosition.x() + m.m13 * aPosition.y() + m.m23 * aPosition.z() + m.m33;
            if ((w != 0) && (w != 1)) {
                result /= w;
            }
        ...
        Vector MultiplyDirection(const Vector &aDirection) const {
            ...
            const float w =
                    m.m03 * aDirection.x() + m.m13 * aDirection.y() + m.m23 * aDirection.z();
            if ((w != 0) && (w != 1)) {
                result /= w;
            }
        ...
        void
        DecomposePerspective(float &aLeft, float &aRight, float &aTop, float &aBottom, float &aNear,
                             float &aFar) {
        ...
        void DecomposePerspectiveDegrees(float &aLeft, float &aRight, float &aTop, float &aBottom,
